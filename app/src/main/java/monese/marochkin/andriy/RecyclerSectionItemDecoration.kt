...
import android.widget.TextView
 ...
 // Decoration for RecyclerView with Sticky header
class RecyclerSectionItemDecoration(private val headerOffset: Int, private val sticky: Boolean, private val sectionCallback: SectionCallback) : RecyclerView.ItemDecoration() {
    ...
    private fun drawHeader(c: Canvas, child: View, headerView: View) {
        c.save()
        if (sticky) {
            c.translate(0f, Math.max(0, child.top - headerView.height-5).toFloat())
        } else {
            c.translate(0f, (child.top - headerView.height-5).toFloat())
        }
        headerView.draw(c)
        c.restore()
    }
